// Migration runner para el módulo search.
// Lee archivos .sql del directorio migrations, parsea las secciones -- +migrate Up,
// y aplica las migraciones pendientes trackeadas en schema_migrations.
package search
...
var searchMigrationFiles embed.FS

const searchSchemaMigrationsTable = `
...
// RunMigrations ejecuta las migraciones pendientes del módulo search.
// Debe llamarse antes de cualquier operación de DB en el search pool.
func RunMigrations(ctx context.Context, pool *pgxpool.Pool) error {
	...
	entries, err := searchMigrationFiles.ReadDir("migrations")
	if err != nil {
	...
	for _, name := range names {
		...
		slog.Info("migration applied", "module", "search", "file", name)
	}
...
func ensureSchemaMigrations(ctx context.Context, pool *pgxpool.Pool) error {
	_, err := pool.Exec(ctx, searchSchemaMigrationsTable)
	return err
}
...
func extractUpSection(filename string) (string, error) {
	...
	data, err := searchMigrationFiles.ReadFile(path)
	if err != nil {
		return "", err
	}
