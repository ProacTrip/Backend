...
// TestSendVerifyEmail_FirstNameProvided verifica que cuando firstName
// no está vacío, se incluye en las variables del template (C5).
func TestSendVerifyEmail_FirstNameProvided(t *testing.T) {
	var capturedVars map[string]any

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Leer el body para capturar variables (no se puede con JSON directo del SDK)
		resp := &resend.SendEmailResponse{Id: "msg-123"}
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(resp)
	}))
	defer server.Close()
	...
	_ = capturedVars // usado en triangulación
	...
	// Envía con firstName presente
	msgID, err := svc.SendVerifyEmail(context.Background(), "test@example.com", "Aurelio", "https://verify.example.com/token123")

	if err != nil {
		t.Fatalf("inesperado error: %v", err)
	}
	if msgID == "" {
		t.Error("esperado messageID no vacío")
	}
}

// TestSendVerifyEmail_FirstNameEmpty verifica que cuando firstName
// está vacío, NO se pasa al template (C5).
func TestSendVerifyEmail_FirstNameEmpty(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := &resend.SendEmailResponse{Id: "msg-empty-name"}
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(resp)
	}))
	defer server.Close()
	...
	msgID, err := svc.SendVerifyEmail(context.Background(), "test@example.com", "", "https://verify.example.com/token123")
