...
var notificationMigrationFiles embed.FS

const notificationSchemaMigrationsTable = `
...
// Debe llamarse antes de cualquier operación de DB en el notification pool.
func RunMigrations(ctx context.Context, pool *pgxpool.Pool) error {
	...
	entries, err := notificationMigrationFiles.ReadDir("migrations")
	if err != nil {
	...
	for _, name := range names {
		...
		slog.Info("migration applied", "module", "notification", "file", name)
	}
...
func ensureSchemaMigrations(ctx context.Context, pool *pgxpool.Pool) error {
	_, err := pool.Exec(ctx, notificationSchemaMigrationsTable)
	return err
}
...
func extractUpSection(filename string) (string, error) {
	...
	data, err := notificationMigrationFiles.ReadFile(path)
	if err != nil {
		return "", err
	}
