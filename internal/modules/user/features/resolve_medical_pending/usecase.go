// Caso de uso: Resolver conflicto médico pendiente (accept/reject/custom).
...
// MedicalPendingRepo permite leer y resolver actualizaciones pendientes.
...
// UseCase implementa la resolución de conflictos médicos pendientes.
type UseCase struct {
	medicalPendingRepo MedicalPendingRepo
	medicalProfileRepo MedicalProfileRepo
	encryptionService  EncryptionSvc
...
func (uc *UseCase) Execute(ctx context.Context, cmd Command) (*ResolveResponse, error) {
	...
	pendingID, err := uuid.Parse(cmd.PendingUpdateID)
	if err != nil {
		return nil, fmt.Errorf("invalid pending_update_id: %w", err)
	}

	// 1. Fetch pending update
	pu, err := uc.medicalPendingRepo.GetByID(ctx, pendingID)
	if err != nil {
		return nil, err
	}
	if pu == nil {
	...
	case "accept":
		...
		if err := uc.setMedicalField(mp, pu.FieldName, pu.ProposedValue, pu.SourceType, now); err != nil {
			return nil, err
		}
		if err := uc.medicalProfileRepo.Update(ctx, mp); err != nil {
			return nil, fmt.Errorf("update medical profile: %w", err)
		}
		if err := uc.medicalPendingRepo.Resolve(ctx, pendingID, domain.PendingUpdateAccepted); err != nil {
			return nil, err
		}
	...
	case "reject":
		...
		if err := uc.medicalPendingRepo.Resolve(ctx, pendingID, domain.PendingUpdateRejected); err != nil {
			return nil, err
		}
	...
	case "custom":
		// Valor personalizado
		if cmd.CustomValue == nil {
			return nil, domain.ErrInvalidPendingAction
		}
		if err := uc.setMedicalField(mp, pu.FieldName, *cmd.CustomValue, pu.SourceType, now); err != nil {
			return nil, err
		}
		if err := uc.medicalProfileRepo.Update(ctx, mp); err != nil {
			return nil, fmt.Errorf("update medical profile: %w", err)
		...
		uc.wg.Go(func() {
			...
			if err != nil {
				slog.WarnContext(bgCtx, "publish medical pending resolved event failed",
					...
					slog.String("pending_id", pendingID.String()),
...
func (uc *UseCase) setMedicalField(mp *domain.MedicalProfileV2, fieldName, value, sourceType string, now time.Time) error {
	...
	if encryptableFields[fieldName] {
		...
		mp.Data[fieldName+"_enc"] = &domain.MedicalFieldValue{
			Value:     encodedValue,
			Source:    domain.MedicalSource(sourceType),
			UpdatedAt: now,
		}
	} else {
